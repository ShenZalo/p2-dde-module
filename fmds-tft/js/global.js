...
// Show and Hide content (Active will be blue)
...
triggers.forEach((trigger, index) => {
  trigger.addEventListener('click', () => {
    ...
    if (activeIndex !== null && activeIndex !== index) {
      ...
      active.classList.add('opacity-0', 'translate-y-2');
      setTimeout(() => active.classList.add('hidden'), 300);
    }

    // activate clicked trigger
    trigger.classList.remove('bg-white');
    trigger.classList.add('bg-blue-500', 'text-white');
...
// Show content (slide-down animation)
